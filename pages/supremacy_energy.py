        ...
        with cols[idx % 4]:
            ...
            with col2:
                if st.button("Delete", key=f"del_proj_{row['Quote_Number']}", type="secondary", use_container_width=True):
                    st.session_state["confirm_delete_quote"] = row["Quote_Number"]
                    st.rerun()  # 強制重新渲染顯示確認表單
            ...
            # ================ 刪除專案確認（移出迴圈，使用全域狀態） ================
if "confirm_del_{row['Quote_Number']}" in st.session_state:
    quote_to_delete = st.session_state["confirm_del_{row['Quote_Number']}"]
    st.warning(f"確定要永久刪除專案 **{quote_to_delete}** 嗎？")

    with st.form(key=f"delete_confirm_{quote_to_delete}"):
        st.write("請確認是否刪除此專案及所有相關借調記錄？")

        col1, col2 = st.columns(2)
        submitted_delete = col1.form_submit_button("✅ 確認刪除", type="primary", use_container_width=True)
        ...
        if submitted_delete:
            try:
                ...
                # 刪除相關借調
                manpower_df = manpower_df[manpower_df["Quote_Number"] != quote_to_delete].reset_index(drop=True)
                conn.update(worksheet="supremacy_manpower", data=manpower_df)

                st.success(f"專案 {quote_to_delete} 及所有借調已刪除！")

                # 強制重新讀取
                projects_df = pd.DataFrame(conn.read(worksheet="supremacy_projects", ttl=0))
                manpower_df = pd.DataFrame(conn.read(worksheet="supremacy_manpower", ttl=0))
            ...
            # 清空確認狀態
            del st.session_state["confirm_del_{row['Quote_Number']}"]
            st.rerun()
