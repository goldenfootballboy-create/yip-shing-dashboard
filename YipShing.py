...
def generate_overview_pdf(specs, project_info, qty):
    ...
    for machine_idx in range(qty):
        ...
        elements.append(Spacer(1, 6))
        elements.append(Paragraph(
            f"燃油冷卻器　　貨源： {spec.get('fuel_cooler_source', '—')}　　負責部門： <font color=red>{spec.get('fuel_cooler_department', '—')}</font>",
            normal))
        elements.append(Paragraph(
        ...
        elements.append(Paragraph(
            ...
            normal))
        elements.append(Paragraph(
            f"斷路器： {spec.get('breaker_type', '—')} {spec.get('breaker_rating', '—')} {spec.get('poles', '—')}　　貨源： {spec.get('breaker_source', '—')}　　負責部門： <font color=red>{spec.get('breaker_department', '—')}</font>",
            normal))
        elements.append(Spacer(1, 12))
        ...
        if parts:
            ...
            for p in parts:
                ...
                if name:
                    elements.append(
                        Paragraph(f"- **{name}**　（貨源：{source}，負責部門：<font color=red>{dept}</font>）", normal))
            elements.append(Spacer(1, 12))
