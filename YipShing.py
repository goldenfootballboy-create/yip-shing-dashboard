            ...
            with tabs[i]:
                ...
                with col1:
                    ...
                    e_frequency = st.selectbox("Frequency(頻率)", ["--", "50Hz", "60Hz"],
                                               index=safe_index(current.get("frequency", "--"), ["--", "50Hz", "60Hz"]),
            ...
            for i in range(qty):
                with tabs[i]:
                    ...
                    with st.expander("Engine & Alternator Group", expanded=True):
                        ...
                        with col_d3:
                            s_alt_heater = st.selectbox("Alternator Heater (交流發電機加熱器)",
                                                        ["--", "Include", "Not Include"], key=f"dlg_alt_heater_{i}")
                    ...
                    with st.expander("Radiator & Base Frame Group", expanded=False):
                        ...
                        with col_title:
                            st.markdown("**Radiator (水箱)**")
                        with col_source:
                            s_rad_source = st.selectbox("貨源", ["--", "HK", "DG"], key=f"dlg_rad_source_{i}",
                                                        label_visibility="collapsed")
                        ...
                        col_r1, col_r2 = st.columns(2)
                        with col_r1:
                            s_radiator_guard = st.selectbox("Radiator Guard (水箱護罩)",
                                                            ["--", "Include", "Not Include"],
                        ...
                        with col_title:
                            st.markdown("**Low water level float switch**")
                        with col_include:
                            s_low_water = st.selectbox("", ["--", "Include", "Not Include"], key=f"dlg_low_water_{i}",
                                                       label_visibility="collapsed")
                        with col_source:
                        ...
                        with col_title:
                            st.markdown("**Base Frame (底架)**")
                        with col_source:
                            s_base_source = st.selectbox("貨源", ["--", "HK", "DG"], key=f"dlg_base_source_{i}",
                                                         label_visibility="collapsed")
                        ...
                        with col_title:
                            st.markdown("**Anti-Vibration Mount**")
                        with col_include:
                            s_avm = st.selectbox("", ["--", "Include", "Not Include"], key=f"dlg_avm_{i}",
                                                 label_visibility="collapsed")
                        with col_source:
                            s_avm_source = st.selectbox("貨源", ["--", "HK", "DG"], key=f"dlg_avm_source_{i}",
                                                        label_visibility="collapsed")
                    ...
                    with st.expander("Container & Control & Circuit Breaker Group", expanded=False):
                        ...
                        with col_title:
                            st.markdown("**Container (貨櫃)**")
                        with col_source:
                            s_cont_source = st.selectbox("貨源", ["--", "HK", "DG"], key=f"dlg_cont_source_{i}",
                                                         label_visibility="collapsed")

                        col_c1, col_c2, col_c3 = st.columns(3)
                        with col_c1:
                            s_cont_size = st.selectbox("Size(呎吋)", ["--", "20'ftHQ", "20'ftGP"],
                                                       index=0 if is_open_or_marine else 0, key=f"dlg_cont_size_{i}")
                        with col_c2:
                            s_cont_type = st.selectbox("Type(種類)", ["--", "FIEO", "Motorized"],
                                                       index=0 if is_open_or_marine else 0, key=f"dlg_cont_type_{i}")
                        with col_c3:
                        ...
                        with col_f1:
                            s_fork_slot = st.selectbox("是否帶叉槽位", ["--", "Yes", "No"],
                                                       index=0 if is_open_or_marine else 0, key=f"dlg_fork_slot_{i}")
                        with col_f2:
                            s_anti_noise = st.selectbox("Anti-Noise(78-80Dba @7M 75% loading)", ["--", "Yes", "No"],
                                                        index=0 if is_open_or_marine else 0, key=f"dlg_anti_noise_{i}")
                        col_i1, col_i2 = st.columns(2)
                        with col_i1:
                            s_internal_silencer = st.selectbox("Internal Silencer (內部消聲器)",
                                                               ["--", "Include", "Not Include"],
                                                               index=0 if is_open_or_marine else 0,
                                                               key=f"dlg_internal_silencer_{i}")
                        with col_i2:
                            s_ss_locks = st.selectbox("304 Stainless Steel Door Locks & Hinges",
                                                      ["--", "Include", "Not Include"],
                                                      index=0 if is_open_or_marine else 0, key=f"dlg_ss_locks_{i}")
                        s_emergency_stop = st.selectbox("Emergency Stop Button (緊急暫停)",
                                                        ["--", "Include", "Not Include"],
                                                        index=0 if is_open_or_marine else 0,
                        ...
                        with col_title:
                            st.markdown("**Panel (控制器)**")
                        with col_source:
                            s_panel_source = st.selectbox("貨源", ["--", "HK", "DG"], key=f"dlg_panel_source_{i}",
                                                          label_visibility="collapsed")
                        ...
                        with col_source:
                            s_co_source = st.selectbox("貨源", ["--", "HK", "DG"], key=f"dlg_co_source_{i}",
                                                       label_visibility="collapsed")
                        ...
                        with col_title:
                            st.markdown("**Circuit Breaker (斷路器)**")
                        with col_source:
                            s_breaker_source = st.selectbox("貨源", ["--", "HK", "DG"], key=f"dlg_breaker_source_{i}",
                                                            label_visibility="collapsed")
                        ...
                        with col_b2:
                            s_breaker_rating = st.text_input("Breaker Rating (斷路器容量)",
                                                             key=f"dlg_breaker_rating_{i}")
                        col_p3, col_p4 = st.columns(2)
                        with col_p3:
                            s_poles = st.selectbox("Poles(極數)", ["--", "3P", "4P"], key=f"dlg_poles_{i}")
                        with col_p4:
                            s_spring_charging = st.selectbox("Spring Charging(斷路器操作)",
                                                             ["--", "Motorized", "Single Usage"],
